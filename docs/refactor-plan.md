...
| 8 | 逐檔 .js → .ts | 觸控冒煙檢查 5 項；型別找出一個真實 bug | ✅ |
...
轉檔本身是機械性的，價值在於編譯器順手抓到的問題。

**一個真實的 bug。** 搖桿處理函式呼叫 `clamp`，但階段 6 把它搬進 `input/` 時，
import 留在 `main.js` 沒跟著搬。**手機上一拖搖桿就拋 `ReferenceError`，
移動完全失效。** 已在行動裝置的瀏覽器情境重現確認，再修正。

那個 bug 出現在 commit `a9e34a2` —— 正是訊息裡寫著「九項輸入檢查全過」的那一個。
它們確實都過了，因為**沒有一項碰到搖桿**。當時有指出這個缺口，但沒有補。
`scripts/smoke-touch.mjs` 現在補上了：模擬行動裝置、拖曳搖桿，並用小地圖的
像素確認角色真的移動（實測 20.3 個小地圖像素），另含觸控普攻與點擊技能格。

**61 個型別斷言消失。** `Entity` 原本宣告成 `[key: string]: unknown`，
代價是核心層幾乎每次存取欄位都要寫 `as number`。收緊型別後全部拿掉。

**五處被型別逼出來的結構改善**，每一項獨立來看都是進步：

- `$()` 改為找不到就拋錯，而不是回傳 `HTMLElement | null` 讓 null 流到下游
- 特效改成以 `expand` 為判別欄位的聯集型別。原本的結構讓「沒有速度的粒子」
  也能通過型別檢查
- HUD 的巨獸欄位原本是兩串四層巢狀三元運算子，型別收窄怎麼都過不了，
  抽成 `bossTitle()` 與 `bossStatus()` 兩個扁平的守衛序列
- 輸入層重複的 `session.state !== "playing" || player.hp <= 0` 收斂成
  `activePlayer()`
- 三處散落的模型釋放規則統一到 `disposeModel()`

**`allowJs` 已關閉**，並由 `tests/architecture.test.ts` 斷言 `src/` 底下
不再出現 `.js`，避免型別覆蓋率在不知不覺中倒退。
